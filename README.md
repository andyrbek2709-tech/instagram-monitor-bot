...
Telegram-бот для мониторинга и анализа постов Instagram через HikerAPI + OpenAI GPT-4o.

## Что умеет

- **Разбор поста по ссылке** — кидаешь ссылку на любой пост/Reel, бот показывает текст и делает анализ через OpenAI GPT-4o на русском языке
...
filter.py        — классификация контента через GPT-4o (реклама/личное)
analyzer.py      — анализ через GPT-4o (sentiment, темы, релевантность)
...
| AI анализ | OpenAI GPT-4o (текст, Vision, Whisper) |
| Контент-фильтр | OpenAI GPT-4o (опционально) |
...
**Опциональные:**
- `OPENAI_API_KEY` — для AI-анализа постов, Vision и транскрипции видео (Whisper)
...
| `filter_results` | результаты GPT-4o-классификации |
...
3. Бот покажет текст и автоматически сделает разбор через OpenAI GPT-4o
