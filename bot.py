...
"""
    ...
    async def create_prompt_action(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            prompt = (
                "Ты — продвинутый AI-архитектор. Проанализируй Instagram пост и создай "
                "КРАЙНЕ конкретный и САМОДСТАТОЧНЫЙ промпт для передачи другому AI-агенту.\n\n"
                "ШАБЛОН ПРОМПТА:\n\n"
                "🔍 РАЗВЕДКА (выполни ДО действия):\n"
                "1. Изучи суть бизнеса/продукта и исходный контент поста\n"
                "2. Поищи существующие решения, библиотеки, технологии\n"
                "3. Анализируй подходы конкурентов и индустрии\n"
                "4. Посмотри GitHub репозитории, лайки, звёзды решений\n"
                "5. Сравни разные подходы, вычисли плюсы и минусы\n\n\n"
                "📊 ДЕЙСТВИЕ ДЛЯ AGENTA (выполни ПОСЛЕ разведки):\n"
                "1. Выбери оптимальный технический стек на основе разведки\n"
                "2. Объясни стойвость каждого подхода (почему работают или нет)\n"
                "3. Опиши технические детали реализации\n"
                "4. Укажи реальные сроки и бюджеты\n"
                "5. Дай плюсы и минусы каждого решения\n\n"
                "Я хочу реализовать следующую бизнес-идею:\n\n"
                "🎯 ЧТО ИМЕННО НУЖНО СДЕЛАТЬ (кратко, но конкретно):\n"
                "🛠 ТЕХНОЛОГИЧЕСКИЙ СТЕК (точные библиотеки, языки):\n"
                "📋 ПОШАГОВЫЙ ПЛАН (что сначала, что потом):\n"
                "⏰ СРОКИ (реалистично):\n"
                "💰 БЮДЖЕТ И ОЦЕНКА (если применимо):\n"
                "🔄 АЛЬТЕРНАТИВНЫЕ ВАРИАНТЫ (бесплатные/платные):\n\n"
                "ОГРАНИЧЕНИЯ:\n"
                "- Максимум 1500 символов для разведки, 1200 для финала\n"
                "- Максимум фактов и конкретики, минимум воды\n"
                "- Ответ должен быть БУКВАЛЬНО готов к исполнению агентом\n"
                "- Агент не должен задавать вопросов пользователю\n\n\n"
                f"--- ИСХОДНЫЙ ПОСТ ---\n"
                f"Аккаунт: {raw_content.get('account', 'Неизвестно')}\n"
                f"Лайки: {raw_content.get('likes', 0)} | Комментарии: {raw_content.get('comments', 0)}\n"
                f"Текст поста (полностью):\n{raw_content.get('caption', 'Нет текста')}\n\n"
                f"--- АНАЛИЗ ИЗ POSTA ---\n{analysis}\n\n"
                "Теперь создай разведочный промпт по шаблону выше."
            )
