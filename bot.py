    ...
    async def create_prompt_action(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            prompt = (
                ...
                "КРАЙНЕ конкретный и ГОТОВ К ИСПОЛЬЗОВАНИЮ промпт для ChatGPT или Gemini.\n\n"
                "СТРОГО по этому шаблону:\n\n"
                ...
                "⏰ СРОКИ (реалистично):\n\n"
                "Ограничения: до 800 символов, МАКСИМУМ конкретики, НИКАКИХ \"оцените\", \"проанализируйте\".\n\n"
                "Важно: промпт должен быть БУКВАЛЬНО готов к копированию и исполнению.\n\n"
                f"--- ИСХОДНЫЙ ПОСТ ---\n"
                ...
                f"--- АНАЛИЗ ---\n{analysis}\n\n"
                "Теперь создай промпт по шаблону выше."
            )
