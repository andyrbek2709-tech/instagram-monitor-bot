...
def _carousel_slide_block(sl: Dict, idx: int, openai_key: str) -> str:
    ...
    vision_prompt = prompts.VISION_CAROUSEL_SLIDE
...
"""
    ...
    async def analyze_url_receive(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        ...
        try:
                        ...
                        messages=[
                            {
                                "role": "user",
                                "content": [
                                    {"type": "text", "text": prompts.VISION_VIDEO_PREVIEW},
                                    {
                                        "type": "image_url",
                                        "image_url": {"url": f"data:image/jpeg;base64,{img_base64}"}
                                    }
            ...
            try:
                ...
                # ВАЖНО: записываем обогащённый caption (Vision + Whisper) обратно в post,
                # иначе реверс-инжиниринг и кнопки анализа увидят пустой текст у видео-постов.
    ...
    async def create_prompt_action(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            ...
            prompt = prompts.build_create_prompt(
                account=raw.get('account', 'Неизвестно'),
                likes=raw.get('likes', 0),
                comments=raw.get('comments', 0),
                caption=raw.get('caption', 'Нет текста'),
                analysis=analysis,
            )
    ...
    async def analyze_post_action(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if not caption:
            await query.edit_message_text(
                "⚠️ Не удалось получить контент поста (ни текста, ни разбора видео).\n\n"
                "Отправь ссылку заново через «Разобрать пост»."
            )
            return

        prompt = prompts.build_quick_action_prompt(action, caption)
    ...
    async def reverse_engineer_action(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            system_prompt = prompts.REVERSE_SYSTEM_PROMPT
            user_prompt = prompts.build_reverse_prompt(
                account=account, likes=likes, comments=comments, caption=caption,
            )
