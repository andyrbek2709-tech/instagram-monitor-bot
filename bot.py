...
"""
    ...
    async def create_prompt_action(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            prompt = (
                "На основе анализа Instagram поста создай ОДИН промпт для передачи AI-агенту (Claude, GPT-4 и т.д.).\n\n"
                "ПРАВИЛА:\n"
                "1. НЕ придумывай технические детали сам — оставь это агенту\n"
                "2. Опиши ЧТО мы хотим сделать и ПОЧЕМУ (суть из поста)\n"
                "3. Дай агенту инструкции для РАЗВЕДКИ: что изучить, где поискать\n"
                "4. Промпт должен быть самодостаточным — агент работает без вопросов\n\n"
                "СТРУКТУРА ПРОМПТА:\n\n"
                "📌 КОНТЕКСТ:\n[Суть идеи из поста — что увидели, в чём ценность, для кого]\n\n"
                "🎯 ЗАДАЧА ДЛЯ AGENTA:\n[Чего мы хотим добиться — без технических деталей]\n\n"
                "🔍 ИНСТРУКЦИИ ПО РАЗВЕДКЕ:\n[Что агент должен изучить: GitHub, решения, конкуренты, подходы]\n\n"
                "📋 ТРЕБОВАНИЯ К РЕЗУЛЬТАТУ:\n[Что должен выдать агент после разведки: стек, план, сроки, цена, альтернативы]\n\n"
                "ОГРАНИЧЕНИЯ: до 1000 символов, только факты, без воды.\n\n"
                f"--- ИСХОДНЫЙ ПОСТ ---\n"
                ...
                f"--- АНАЛИЗ ---\n{analysis}\n\n"
                "Создай промпт по структуре выше. НЕ придумывай стеки и решения — оставь разведку агенту."
            )
